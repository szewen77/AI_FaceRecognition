# classifiers/knn_classifier.py - KNN-based face classifier
import numpy as np
import pickle
from sklearn.neighbors import KNeighborsClassifier
from sklearn.preprocessing import LabelEncoder
from sklearn.metrics.pairwise import cosine_similarity
from face_core import BaseClassifier

class KNNClassifier(BaseClassifier):
    """KNN-based face classifier with cosine similarity fallback"""
    
    def __init__(self, n_neighbors=5, weights='distance', metric='euclidean', similarity_weight=0.5):
        """
        Initialize KNN Classifier
        
        Args:
            n_neighbors: Number of neighbors to use for KNN
            weights: Weight function used in prediction ('uniform', 'distance')
            metric: Distance metric for KNN ('euclidean', 'manhattan', 'cosine', etc.)
            similarity_weight: Weight for cosine similarity in final confidence
        """
        self.n_neighbors = n_neighbors
        self.weights = weights
        self.metric = metric
        self.similarity_weight = similarity_weight
        
        self.knn_model = None
        self.label_encoder = None
        self.known_embeddings = []
        self.known_names = []
        self.is_single_person = False
    
    def train(self, embeddings, names):
        """
        Train KNN classifier
        
        Args:
            embeddings: List of face embeddings
            names: List of corresponding names
            
        Returns:
            bool: Success status
        """
        if len(embeddings) == 0:
            print("No embeddings provided for training")
            return False
        
        # Store embeddings for similarity calculation
        self.known_embeddings = embeddings.copy()
        self.known_names = names.copy()
        
        # Prepare data
        X = np.array(embeddings)
        y = np.array(names)
        
        # Check number of unique classes
        unique_classes = len(np.unique(y))
        
        if unique_classes == 1:
            print(f"Single person mode: {np.unique(y)[0]}")
            print("Using distance-based recognition only")
            self.is_single_person = True
            self.label_encoder = LabelEncoder()
            self.label_encoder.fit(y)
            return True
        
        # Multiple people - train KNN
        self.is_single_person = False
        
        # Encode labels
        self.label_encoder = LabelEncoder()
        y_encoded = self.label_encoder.fit_transform(y)
        
        # Adjust n_neighbors if necessary
        effective_neighbors = min(self.n_neighbors, len(embeddings))
        if effective_neighbors < self.n_neighbors:
            print(f"Reducing n_neighbors from {self.n_neighbors} to {effective_neighbors} due to limited training data")
        
        # Train KNN
        self.knn_model = KNeighborsClassifier(
            n_neighbors=effective_neighbors,
            weights=self.weights,
            metric=self.metric,
            algorithm='auto'
        )
        
        try:
            self.knn_model.fit(X, y_encoded)
            print(f"KNN trained successfully with {unique_classes} classes")
            print(f"Classes: {list(self.label_encoder.classes_)}")
            print(f"Parameters: n_neighbors={effective_neighbors}, weights={self.weights}, metric={self.metric}")
            return True
        except Exception as e:
            print(f"KNN training failed: {e}")
            return False
    
    def predict(self, embedding):
        """
        Predict identity from embedding
        
        Args:
            embedding: Face embedding vector
            
        Returns:
            tuple: (predicted_name, confidence)
        """
        if self.label_encoder is None:
            return "Unknown", 0.0
        
        # Reshape for prediction
        embedding = embedding.reshape(1, -1)
        
        # Calculate cosine similarities
        similarities = []
        for known_embedding in self.known_embeddings:
            similarity = cosine_similarity(
                embedding, 
                np.array(known_embedding).reshape(1, -1)
            )[0][0]
            similarities.append(similarity)
        
        max_similarity = max(similarities) if similarities else 0.0
        best_similarity_idx = np.argmax(similarities) if similarities else 0
        
        # Single person mode
        if self.is_single_person:
            predicted_name = self.label_encoder.classes_[0]
            return predicted_name, max_similarity
        
        # Multi-person mode with KNN
        if self.knn_model is None:
            return "Unknown", 0.0
        
        try:
            # Get KNN prediction
            predicted_class = self.knn_model.predict(embedding)[0]
            predicted_name = self.label_encoder.inverse_transform([predicted_class])[0]
            
            # Get KNN distances/probabilities for confidence estimation
            if hasattr(self.knn_model, 'predict_proba'):
                try:
                    probabilities = self.knn_model.predict_proba(embedding)[0]
                    knn_confidence = np.max(probabilities)
                except:
                    # Some metrics don't support predict_proba, fallback to distance-based confidence
                    distances, indices = self.knn_model.kneighbors(embedding)
                    # Convert distance to confidence (closer = more confident)
                    # Normalize distance to confidence score between 0 and 1
                    max_distance = np.max(distances) if np.max(distances) > 0 else 1.0
                    min_distance = np.min(distances)
                    knn_confidence = 1.0 - (min_distance / max_distance)
            else:
                # Fallback: use distance-based confidence
                distances, indices = self.knn_model.kneighbors(embedding)
                avg_distance = np.mean(distances)
                # Convert to confidence score (lower distance = higher confidence)
                knn_confidence = 1.0 / (1.0 + avg_distance)
            
            # Combine KNN confidence and cosine similarity
            final_confidence = (
                (1 - self.similarity_weight) * knn_confidence + 
                self.similarity_weight * max_similarity
            )
            
            return predicted_name, final_confidence
            
        except Exception as e:
            print(f"KNN prediction error: {e}")
            # Fallback to similarity-based prediction
            if similarities:
                best_name = self.known_names[best_similarity_idx]
                return best_name, max_similarity
            return "Unknown", 0.0
    
    def save(self, filepath):
        """Save trained classifier"""
        model_data = {
            'knn_model': self.knn_model,
            'label_encoder': self.label_encoder,
            'known_embeddings': self.known_embeddings,
            'known_names': self.known_names,
            'is_single_person': self.is_single_person,
            'n_neighbors': self.n_neighbors,
            'weights': self.weights,
            'metric': self.metric,
            'similarity_weight': self.similarity_weight
        }
        
        try:
            with open(filepath, 'wb') as f:
                pickle.dump(model_data, f)
            print(f"KNN classifier saved to {filepath}")
            return True
        except Exception as e:
            print(f"Error saving KNN classifier: {e}")
            return False
    
    def load(self, filepath):
        """Load trained classifier"""
        try:
            with open(filepath, 'rb') as f:
                model_data = pickle.load(f)
            
            self.knn_model = model_data['knn_model']
            self.label_encoder = model_data['label_encoder']
            self.known_embeddings = model_data['known_embeddings']
            self.known_names = model_data['known_names']
            self.is_single_person = model_data['is_single_person']
            self.n_neighbors = model_data.get('n_neighbors', 5)
            self.weights = model_data.get('weights', 'distance')
            self.metric = model_data.get('metric', 'euclidean')
            self.similarity_weight = model_data.get('similarity_weight', 0.5)
            
            print(f"KNN classifier loaded from {filepath}")
            return True
        except Exception as e:
            print(f"Error loading KNN classifier: {e}")
            return False
    
    def get_name(self):
        """Return classifier name"""
        return "KNN"
    
    def get_parameters(self):
        """Get current parameters"""
        return {
            'n_neighbors': self.n_neighbors,
            'weights': self.weights,
            'metric': self.metric,
            'similarity_weight': self.similarity_weight,
            'is_single_person': self.is_single_person,
            'num_classes': len(set(self.known_names)) if self.known_names else 0,
            'total_samples': len(self.known_embeddings)
        }
    
    def set_similarity_weight(self, weight):
        """Adjust the weight of cosine similarity in final prediction"""
        self.similarity_weight = max(0.0, min(1.0, weight))
        print(f"Similarity weight set to {self.similarity_weight}")
    
    def set_n_neighbors(self, n_neighbors):
        """Adjust the number of neighbors for KNN"""
        if n_neighbors > 0:
            self.n_neighbors = n_neighbors
            print(f"n_neighbors set to {self.n_neighbors}")
            # Note: You'll need to retrain if you want this to take effect
            print("Note: Retrain the model for this change to take effect")
        else:
            print("n_neighbors must be positive")
