# main_multiclassifier.py - Face Recognition with Multiple Classifier Support
import argparse
import sys
...
def create_classifier(classifier_type, args):
    """Create a classifier based on type and arguments"""
    if classifier_type.upper() == 'SVM':
        return SVMClassifier(
            kernel=getattr(args, 'svm_kernel', 'linear'),
            C=getattr(args, 'svm_C', 1.0),
            similarity_weight=getattr(args, 'similarity_weight', 0.5)
        )
    elif classifier_type.upper() == 'KNN':
        return KNNClassifier(
            n_neighbors=getattr(args, 'k_neighbors', 5),
            weights=getattr(args, 'knn_weights', 'distance'),
            metric=getattr(args, 'knn_metric', 'euclidean'),
            similarity_weight=getattr(args, 'similarity_weight', 0.5)
        )
    else:
        raise ValueError(f"Unknown classifier type: {classifier_type}")

def compare_classifiers(system, test_folder=None):
    """Compare different classifiers on the same data"""
    if not system.known_embeddings:
        print("No enrolled data found for comparison. Please enroll people first.")
        return
    
    print("\n=== Classifier Comparison ===")
    
    # Test both SVM and KNN
    classifiers_to_test = [
        ('SVM', SVMClassifier(kernel='linear', C=1.0, similarity_weight=0.5)),
        ('KNN-5', KNNClassifier(n_neighbors=5, weights='distance', metric='euclidean', similarity_weight=0.5)),
        ('KNN-3', KNNClassifier(n_neighbors=3, weights='distance', metric='euclidean', similarity_weight=0.5)),
        ('KNN-Cosine', KNNClassifier(n_neighbors=5, weights='distance', metric='cosine', similarity_weight=0.5))
    ]
    
    original_classifier = system.classifier
    results = {}
    ...
    for classifier_name, classifier in classifiers_to_test:
        ...
        success = system.switch_classifier(classifier)
        if success:
            # You could add test evaluation here if you have test data
            params = classifier.get_parameters()
            results[classifier_name] = {
                'trained': True,
                'parameters': params,
                'num_classes': params.get('num_classes', 0),
                'total_samples': params.get('total_samples', 0)
            }
            print(f"✅ {classifier_name} trained successfully")
            print(f"   Classes: {params.get('num_classes', 0)}, Samples: {params.get('total_samples', 0)}")
        else:
            results[classifier_name] = {'trained': False}
            print(f"❌ {classifier_name} training failed")
    
    # Revert to original classifier
    system.switch_classifier(original_classifier)
    
    print(f"\n=== Comparison Summary ===")
    for name, result in results.items():
        if result['trained']:
            print(f"{name}: ✅ Ready (Classes: {result['parameters'].get('num_classes', 0)})")
        else:
            print(f"{name}: ❌ Failed")

def main():
    parser = argparse.ArgumentParser(description='Face Recognition Attendance System with Multiple Classifiers')
    
    # Basic operations
    parser.add_argument('--mode', choices=['enroll', 'run', 'report', 'compare'], 
                       default='run', help='Operation mode')
    
    # Classifier selection
    parser.add_argument('--classifier', choices=['SVM', 'KNN'], default='KNN',
    ...
    classifier = create_classifier(args.classifier, args)
    ...
    print(f"Initialized Face Recognition System with {args.classifier}")
    if args.classifier.upper() == 'SVM':
        print(f"SVM Parameters: kernel={args.svm_kernel}, C={args.svm_C}")
    elif args.classifier.upper() == 'KNN':
    ...
    if args.mode == 'enroll':
        ...
        print(f"Starting live attendance system with {args.classifier}...")
        print("Press 'q' to quit, 's' to save screenshot")
        system.run_live_attendance()
    
    elif args.mode == 'compare':
        compare_classifiers(system)
        ...
        if not df.empty:
            ...
            if 'classifier_type' in df.columns:
                ...
                for classifier, count in classifier_usage.items():
                    print(f"  {classifier}: {count} records")
        ...
        filename = f"attendance_report_{args.classifier.lower()}_{timestamp}.csv"
