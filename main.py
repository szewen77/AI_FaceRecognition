# main_app.py - Simplified Face Recognition with SVM only
import argparse
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Face Recognition Attendance System with SVM')
    
    # Basic operations
    parser.add_argument('--mode', choices=['enroll', 'run', 'report', 'performance'], 
    ...
    print(f"Initialized Face Recognition System with SVM")
    print(f"SVM Parameters: kernel={args.svm_kernel}, C={args.svm_C}, similarity_weight={args.similarity_weight}")
    ...
    if args.mode == 'enroll':
        ...
        print("Starting live attendance system...")
        ...
        if not system.known_embeddings:
            print("No enrolled data found for performance analysis.")
            print("Please enroll people first using: python main_app.py --mode enroll --batch-folder <folder>")
            return
        ...
        try:
            ...
            print(f"Performance analysis module not found: {e}")
            print("Make sure performance_analyzer.py is in the same directory.")
